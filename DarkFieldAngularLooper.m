...
tic
%%
%set variables - please check the following every run!
%--------------------------------------------------------------------------
%set your phone number for updates about your code ! Please -  if you are
%not me then change this, because otherwise it will text me about your code
%and I will track you down about it - gh 8-8-15
phoneNumber = '505-379-5053';
carrier = 'Verizon';

%get master directory as user input
referenceFrame = 20;%taken by looking at the data set. Change w each time!
...
timeDelay = 31;%this is the amount of time elapsed from plating to the 
%first image being taken. It's really crucial that you keep careful track
%of this for faithful comparison between data sets. Units are minutes
%--------------------------------------------------------------------------

%you can generally leave the following alone unless you feel like messing
%around more
%--------------------------------------------------------------------------
...
saveDir = uigetdir (cd, 'Pick Directory for Save Information');
...
for i = 1:size(plateDir,1)
    ...
    for j = 1:6
         ...
         if i ==1 && j==1 
            angularMovieBool = true;
         else
             angularMovieBool = false;
         end
         
        %get the data for the given well
        [wellData(1).times, wellData(1).waveData, ...
            wellData(1).period, ...
            ...
            significanceLevel, angularMovieBool, timeDelay);
%--------------------------------------------------------------------------     
...
%make and name a folder to store figures in
mkdir(saveDir, 'Figures');
figureDir = [saveDir filesep 'Figures'];

%get means/stds and visualize data
[perMeanTot, perSTDTot, timeMeanTot, timeSTDTot, totHist] = ...
    PeaksStatistics (totalPeaks, true, figureDir);
...
[meanExperimentPower, meanExperimentSTD, meanExperimentStErr] = ...
    MeanPowerAnalysis(totPower, totSTD, totStErr, ...
     ...
     referenceTime,perStart, perEnd, timeStart, figureDir);
...
% catch ME
%     messageString = ME.message;
%     send_text_message (phoneNumber, carrier, '', ...
%         ['Oh No :( !! Your Code Crashed!!!!!!!!!! ' messageString ...
%         ' at ' ME.stack.line])
%     rethrow (ME);
% end 
%  send_text_message (phoneNumber, carrier, '', ...
%      'Congradulation! Your code is done running!');
